...
const Days = ({date, startDate, endDate, onClick }) => {
    ...
    for (let i = 1; i <= 7; i++) {
        labels.push(React.createElement("span", {className: "label"},
        moment().day(i).format('ddd')));
    }

    for (let i = firstDayDate.day(); i > 1; i--) {
        days.push(
        React.createElement(Day, {key: moment(thisDate).format('DD MM YYYY'), onClick: date =>
    ...
    for (let i = 1; i <= 42 - daysCount; i++) {
        nextsMonth.date(i);
        days.push(
        React.createElement(Day, {key: moment(nextsMonth).format('DD MM YYYY'), onClick: date =>
